...
## Intel Graphics
* Introduced initial performance optimizations for future integrated GPUs based on Xe3p-LPG architecture. This functionality is not dispatched by default and requires opt-in with environment variable `ONEDNN_ENABLE_PRIMITIVE_GPU_ISA=XE3P`. 
* Introduced initial performance optimizations for future discrete GPUs based on Xe3p-XPC architecture. This functionality is not dispatched by default and requires opt-in with environment variable `ONEDNN_ENABLE_PRIMITIVE_GPU_ISA=XE3P`. 
