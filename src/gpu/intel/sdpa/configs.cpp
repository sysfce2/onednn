...
fwd_config_t *choose_config(compute::gpu_arch_t arch, dim_t head_size,
    ...
    if (it != end(sorted_configs)) {
        ...
        return &it->config;
			...
			VDEBUGINFO(4, primitive, sdpa,
					"config search failed: {query %s},",
					to_string(query).c_str());
        }
        switch (arch_query) {
        case compute::gpu_arch_t::xe3p: arch_query = compute::gpu_arch_t::xe3;
            break;
        case compute::gpu_arch_t::xe3: arch_query = compute::gpu_arch_t::xe2;
            break;
        case compute::gpu_arch_t::xe2: arch_query = compute::gpu_arch_t::xe_hpc;
            break;
        case compute::gpu_arch_t::xe_hpc: fallback = false;
            break;
        default: 
