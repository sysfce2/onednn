...
status_t init_brgemm_matmul_conf(cpu_isa_t isa, brgemm_matmul_conf_t &bgmmc,
    ...
    if (has_wei_zp) {
        ...
        // For int8 grouped quantization with grouped weight zero points, the
        // copy_b kernel centers the byte via `vpsubb(wei, zp)`. To keep the
        // resulting byte within s8 range (so the AMX/VNNI signed dot product
        // produces correct results) the original wei dt must be int4 and the
        // ZP dt must match the original wei dt exactly:
        //   wei=s4 ⇒ ZP must be s4 (centered byte ∈ [-15, 15])
        //   wei=u4 ⇒ ZP must be u4 (centered byte ∈ [-15, 15])
        // Other combinations (int8 wei, mismatched int4 wei/ZP, etc.) cannot
        // fit the centered value in a signed byte and are not supported on
        // this path.
        const bool is_grouped_wei_zp
                = bgmmc.is_wei_zp_per_k || bgmmc.is_wei_zp_per_n;
        VCONDCHECK_BG(
                IMPLICATION(bgmmc.with_int8_grouped_quantization
                                && is_grouped_wei_zp,
                        one_of(bgmmc.orig_wei_dt, data_type::s4, data_type::u4)
