...
struct brgemm_desc_t {
    ...
    // Per-K-group flag: when true, the caller has shifted `ptr_wei_scales` to
    // the current K-group's per-N scale vector. Brgemm applies per-N scales
    // exactly as for `is_per_n_wei_scales` but at C-accumulation time of every
    // brgemm call (instead of only at post-ops). Used by the matmul int8
    // grouped quantization path where K_blk == group_size and one brgemm call
