...
bool jit_brgemm_amx_uker_base_t::maybe_pre_process_data(brgemm_iteration_t &bi,
    ...
    const bool fp8_fp8_via_convert = brg.is_fp8 && brg.is_fp8_via_convert();
    ...
    auto should_save_transform = [&](matrix_kind_t mk) {
        ...
        if (fp8_fp8_via_convert) return false;
    ...
    // a and b can share the same buffer in case of fp8_fp8_via_convert;
    // a is not converted in case of is_xf16_fp8, so no need for buffer for a.
    const auto scratch_a_tiles = fp8_fp8_via_convert || brg.is_xf16_fp8
            ? 0
