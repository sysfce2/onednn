...
struct jit_lnorm_stat_and_data_kernel_t : stat_and_data_kernel_iface_t,
    ...
    jit_lnorm_stat_and_data_kernel_t(const layer_normalization_pd_t *pd)
        ...
        io::io_tail_conf_t io_tail_conf(simd_w_, axis_simd_tail_, tail_opmask_,
                static_cast<int>(z_tail_mask_.getIdx()), reg_tmp_, reg_tmp1_);
        typename io::jit_io_multi_dt_helper_t<Vmm>::data_types_t io_dts {
        ...
        saturation_map.emplace(dst_d_.data_type(),
                io::io_saturation_conf_t(static_cast<int>(z_zero_.getIdx()),
                        static_cast<int>(z_saturation_ubound_.getIdx()),
                        reg_tmp_));
        io_ = utils::make_unique<io::jit_io_multi_dt_helper_t<Vmm>>(
        ...
        if (with_postops_) {
            ...
            const binary_injector::rhs_arg_static_params_t binary_rhs_params {
                    static_cast<size_t>(z_po_helper_.getIdx()), reg_po_addr_,
                    reg_po_helper_, reg_po_cache_, preserve_gpr, preserve_vmm,
                    static_cast<size_t>(
    ...
    const XReg reg_tmp_ = x15;
    const XReg reg_tmp1_ = x16;
    ...
    const ZReg z_tail_mask_ = ZReg(0);
    const ZReg z_zero_ = ZReg(1);
    const ZReg z_saturation_ubound_ = ZReg(2);
    const ZReg z_qscale_ = ZReg(3);
    const ZReg z_scale_ = ZReg(4);
    const ZReg z_shift_ = ZReg(5);
    const ZReg z_acc_ = ZReg(6);
    const ZReg z_mean_ = ZReg(7);
    const ZReg z_inv_sqrtvar_ = ZReg(8);
    const ZReg z_tmp_ = ZReg(9);
    const ZReg z_dst_ = ZReg(10);
    const ZReg z_po_helper_ = ZReg(11);
    const ZReg z_c_ = ZReg(12);
    const ZReg z_eps_ = ZReg(13);
    const ZReg z_ones_ = ZReg(14);

    std::unique_ptr<io::jit_io_multi_dt_helper_t<Vmm>> io_;
    ...
    void hreduce_sum(const ZReg &zmm) {
        faddv(SReg(zmm.getIdx()), P_ALL_ONE, zmm.s);
        dup(zmm.s, zmm.s[0]);
    }

    void maybe_compute_postops(size_t offt_elems, bool tail) {
        if (!with_postops_) return;

        add_imm(reg_po_addr_, reg_dst_,
        ...
        if (with_binary_) {
            ...
            rhs_arg_params.vmm_idx_to_out_reg.emplace(
                    z_dst_.getIdx(), reg_po_addr_);
            rhs_arg_params.vmm_idx_to_out_elem_off_val.emplace(
                    z_dst_.getIdx(), 0);
            if (tail) rhs_arg_params.vmm_tail_idx_.emplace(z_dst_.getIdx());
            postops_injector_->compute_vector(z_dst_.getIdx(), rhs_arg_params);
        } else {
            postops_injector_->compute_vector(z_dst_.getIdx());
        }
    }

    void calculate_dst_body(size_t offt_elems, bool tail) {
        if (use_scale_) {
            io_->at(f32)->load(
        ...
        if (!skip_mean_) fsub(z_dst_.s, z_dst_.s, z_mean_.s);
        fmul(z_dst_.s, z_dst_.s, z_inv_sqrtvar_.s);

        if (use_scale_) fmul(z_dst_.s, z_dst_.s, z_scale_.s);
        if (use_shift_) fadd(z_dst_.s, z_dst_.s, z_shift_.s);

        if (with_src_scales_) {
            ld1rw(z_qscale_.s, P_ALL_ONE / T_z, ptr(reg_src_scales_));
            fmul(z_dst_.s, z_dst_.s, z_qscale_.s);
        }

        maybe_compute_postops(offt_elems, tail);

        if (with_dst_scales_) {
            ld1rw(z_qscale_.s, P_ALL_ONE / T_z, ptr(reg_dst_scales_));
            fmul(z_dst_.s, z_dst_.s, z_qscale_.s);
        }
    ...
    void compute_mean() {
        ...
        for (dim_t i = 0; i < axis_simd_full_; ++i) {
            io_->at(src_d_.data_type())
                    ->load(addr_with_offt(reg_src_,
                                   i * simd_w_ * src_d_.data_type_size()),
                            0, z_tmp_, false);
        ...
        hreduce_sum(z_acc_);
        uni_fdiv(z_mean_.s, z_acc_.s, z_c_.s, z_tmp_.s, P_ALL_ONE);
        if (save_stats_) str(SReg(z_mean_.getIdx()), ptr(reg_mean_));
    }
        ...
        for (dim_t i = 0; i < axis_simd_full_; ++i) {
            io_->at(src_d_.data_type())
                    ->load(addr_with_offt(reg_src_,
                                   i * simd_w_ * src_d_.data_type_size()),
                            0, z_tmp_, false);
            if (!skip_mean_) fsub(z_tmp_.s, z_tmp_.s, z_mean_.s);
            fmla(z_acc_.s, P_ALL_ONE / T_z, z_tmp_.s, z_tmp_.s);
        }
        if (axis_simd_tail_) {
            io_->at(src_d_.data_type())
                    ->load(addr_with_offt(reg_src_,
                                   axis_simd_full_ * simd_w_
                                           * src_d_.data_type_size()),
                            0, z_tmp_, true);
            if (!skip_mean_) fsub(z_tmp_.s, tail_opmask_ / T_z, z_mean_.s);
            fmla(z_acc_.s, P_ALL_ONE / T_z, z_tmp_.s, z_tmp_.s);
        }
        hreduce_sum(z_acc_);
        uni_fdiv(z_acc_.s, z_acc_.s, z_c_.s, z_tmp_.s, P_ALL_ONE);
        if (save_stats_) str(SReg(z_acc_.getIdx()), ptr(reg_var_));
        mov(z_inv_sqrtvar_.d, z_acc_.d);
    }
...
#define PARAM_OFF(x) static_cast<int32_t>(offsetof(ker_args_t, x))
        ...
        ldr(reg_block_end_, ptr(reg_param_, PARAM_OFF(block_size)));
        add_imm(X_DEFAULT_ADDR, reg_param_, PARAM_OFF(eps), X_TMP_0);
        ldr(W_TMP_0, ptr(X_DEFAULT_ADDR));
#undef PARAM_OFF

        dup(z_eps_.s, W_TMP_0);
        fmov(z_ones_.s, 1.0);
        mov(W_TMP_0, float2int(static_cast<float>(C_)));
        dup(z_c_.s, W_TMP_0);
        ...
        if (calculate_stats_) {
            ...
            if (!skip_mean_) ld1rw(z_mean_.s, P_ALL_ONE / T_z, ptr(reg_mean_));
            ld1rw(z_inv_sqrtvar_.s, P_ALL_ONE / T_z, ptr(reg_var_));
        }

        fadd(z_inv_sqrtvar_.s, z_inv_sqrtvar_.s, z_eps_.s);
        fsqrt(z_inv_sqrtvar_.s, P_ALL_ONE / T_z, z_inv_sqrtvar_.s);
        uni_fdiv(z_inv_sqrtvar_.s, z_ones_.s, z_inv_sqrtvar_.s, z_tmp_.s,
                P_ALL_ONE);
...
template struct jit_uni_layer_normalization_fwd_t<sve_512>;
template struct jit_uni_layer_normalization_fwd_t<sve_256>;
template struct jit_uni_layer_normalization_fwd_t<sve_128>;
